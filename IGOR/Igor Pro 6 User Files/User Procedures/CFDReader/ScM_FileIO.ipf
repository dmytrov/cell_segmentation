...
//	Author		: Thomas Euler
//	Copyright	: (C) MPImF/Heidelberg, CIN/Uni Tübingen 2009-2011
...
constant		SCMIO_UserParameterCount				= 13
...
constant		SCMIO_Param_lastEntry					= 3
...
function LoadSMPFileWithDialog ()
	...
	wSCIOParams		= 0
	wSCIOParams	[SCMIO_Param_addCFDNote]			= 0
	wSCIOParams	[SCMIO_Param_integrStim]			= doIntegrStim
	wSCIOParams	[SCMIO_Param_integrStim_StimCh]	= 3
	wSCIOParams	[SCMIO_Param_integrStim_TargetCh]	= 0
	doLog			= 1
...
function	ScMIO_NewSMPDataFolder (sDFSMP)
	...
	Make/T/O/N=13 $(SCMIO_StrParamWave)
	...
	Make/O/N=24 $(SCMIO_NumParamWave)	
	...
	//	nPixB	= pwNP[%NumberOfPixBufsSet] -pwNP[%PixBufCounter]	
		...
		for(iPixBPerCh=0; iPixBPerCh<nPixB; iPixBPerCh+=1)
			for(iInCh=0; iInCh<SCMIO_maxInputChans; iInCh+=1)
				...
				//	FStatus fileHnd
				//	print iPixB, iPixB*PixBLen, V_filePos/8, V_logEOF/8
					DoUpdate
				endif
			endfor
		endfor		
